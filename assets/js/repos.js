...
function Skins(key) {
  this.init = function() {
    var repos = localStorage.getItem(key);
    if (repos === null || repos === "[]") {
      localStorage.setItem(key, '["/json/skins/default.json"]');
    }
    ...
    if (repo_store === null || repo_store == "{}") {
      localStorage.setItem("current" + key.substring(0, 1).toUpperCase() + key.substring(1, key.length - 1), '{"name":"Default","logo":"/assets/img/icon_mobFull.png","author":"Eclipse Team","description":"The default theme for Eclipse.","styles":[{}]}');
    }
  }
